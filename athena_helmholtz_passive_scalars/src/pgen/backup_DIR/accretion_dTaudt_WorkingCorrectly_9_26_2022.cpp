...
static Real mu, rho_0, rho_f, v_f, p_f, Mdot, r_0, inv_r2, Na, Tc, B_0, alpha, t_rho0_0;
...
static int IDT1, IDT2, IDT3, IDT4, IDT5, IDT6, IDT7;
...
static int nthcycle;
// Optical depth, time for transient to 'go away'
Real global_tau, tau_tolerance;
...
void Double_NR(Real Arr[], Real ra, Real rho0) {
  ...
  for(int i=0; i<=maxcdNR; i++){
    ...
    Real Source0    = YeSource(T0*kbol_MeV,Y0,x_0,rho0);
    Real Source0_dT = YeSource((T0+deltaT0)*kbol_MeV,Y0,x_0,rho0);
    Real Source0_dY = YeSource(T0*kbol_MeV,Y0+deltaY0,x_0,rho0);

    // Define 'original' source functions
    Real qdot    = (qdotScheck_H(Y0,ra) - qdotScheck_C(T0,Y0,rho0,ra));
    Real Yedot   = (YeSource(T0*kbol_MeV,Y0,x_0,rho0) - YeSink(T0*kbol_MeV,Y0,x_0,rho0));
    ...
    Real dYedot_T = (YeSource((T0+deltaT0)*kbol_MeV,Y0,x_0,rho0) - YeSink((T0+deltaT0)*kbol_MeV,Y0,x_0,rho0));
    Real dYedot_Y = (YeSource(T0*kbol_MeV,Y0+deltaY0,x_0,rho0) - YeSink(T0*kbol_MeV,Y0+deltaY0,x_0,rho0));
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  tau_tolerance = pin->GetReal("problem","Tau_Tolerance");
...
//  std::cout << "p_eq  = " << p_0 << " erg/cm^3" << std::endl;
  ...
  if (COMP_DT) {
      ...
      SetUserOutputVariableName(IDT7, "dt4");
  }
}
...
void InflowInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
  ...
  AthenaArray<Real>& my_data4 = pmb->pmy_mesh->ruser_mesh_data[8];

  Real &rho_old{my_data2(0)};
  Real &tau_old{my_data3(0)};
  Real &time_old{my_data4(0)};
  ...
  if(ncycle == 0.0) {
    ...
    dtau_dt = (time - time_old) / (tau - tau_old);
//    std::cout << "------------------------" << std::endl;
...
//    std::cout << "tau_old     = " << tau_old << std::endl;
  }
  ...
  // wait for transient properties to clear
  if(dtau_dt<tau_tolerance) {
    // perturb base density every n'th cycle
    if (Roundncycle == ncycle) {
      if (Roundncycle % nthcycle == 0) {
        if (time != 0.0) {
          if ((tau > tau_v && tau < tau_v * (1.0 + tau_epsilon)) || (tau > tau_v * (1.0 - tau_epsilon) && tau < tau_v)){
            rho_new = rho_old;
  //          prim(IDN,ks,js,is) = rho_new;
  //          rho_new = prim(IDN,js,ks,is);
  ...
  //          prim(IDN,ks,js,is) = rho_new;
    ...
    //      std::cout << "--------------------" << std::endl;
    //      std::cout << "Roundncycle = " << Roundncycle << std::endl;
    //      std::cout << "rho_old     = " << rho_old << " g/cm^3" << std::endl;
    //      std::cout << "--------------------" << std::endl;
      }
    }
  } else {
...
//  std::cout << "t(Inflow)    = " << time << " s" << std::endl;
//  std::cout << "tau(Inflow)  = " << tau << std::endl;
//  std::cout << "rho(Inflow)  = " << prim(IDN,ks,js,is) << std::endl;

// Performing Double NR for Yeq and Teq

//  std::cout << "rho_new (outside) = " << rho_new << " g/cm^3" << std::endl;
  Real r_a   = pco->x1v(is);
  Real rho_a = prim(IDN,ks,js,is);
  Real Arr[2];
//  Double_NR(Arr,r_a,rho_a);
  ...
  Ye_eq          = Arr[1];
  Real* Yeq_ptr  = &Ye_eq;
//  p_0            = pmb->peos->PresFromRhoT(rho_a, T_eq, Yeq_ptr);
//  dpdd_0         = pmb->peos->AsqFromRhoP(rho_a, p_0, Yeq_ptr);
...
//    std::cout << "global_tau      = " << global_tau << std::endl;
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      ...
      for(int i=is; i<=ie; i++) {
        ...
        // Sound Speed
        user_out_var(6,k,j,i) = std::sqrt(peos->AsqFromRhoP(phydro->w(IDN,k,j,i),
