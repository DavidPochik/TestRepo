...
void InflowInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
  ...
  Real &rho0_old{my_data2(0)};
  Real &rho_old{my_data3(0)};
    ...
    if (Roundncycle % nthcycle == 0) {
      if (time != 0.0) {
        if (tau < tau_v * (1.0 + tau_epsilon) && tau > tau_v * (1.0 - tau_epsilon)){
          rho_new  = prim(IDN,ks,js,is);
          rho0_new = rho0_old;
          std::cout << "converged" << std::endl;
          std::cout << "ncycle          = " << ncycle << std::endl;
          std::cout << "rho_new(active) = " << rho_new << " g/cm^3" << std::endl;
          std::cout << "rho_new(ghost)  = " << rho0_new << " g/cm^3" << std::endl;
          std::cout << "tau             = " << tau << std::endl;
        } else if (tau < tau_v * (1.0 - tau_epsilon)) {
          rho_new            = prim(IDN,ks,js,is)  * (1.0 + rho_epsilon);
          rho0_new           = rho0_old * (1.0 + rho_epsilon);
      ...
      if (time == 0.0 || ncycle < 1.0) {
        ...
        // Use current active zone and previous ghost zone values (this assumes that an HSE profile isn't being set in the ghost zones).
        rho_new  = prim(IDN,ks,js,is);
        rho0_new = rho0_old;
      }
    }
...
//  std::cout << "rho_new (outside) = " << rho_new << " g/cm^3" << std::endl;
  Real r_a   = pco->x1v(is);
//  Real rho_a = rho_new; //prim(IDN,ks,js,is);
