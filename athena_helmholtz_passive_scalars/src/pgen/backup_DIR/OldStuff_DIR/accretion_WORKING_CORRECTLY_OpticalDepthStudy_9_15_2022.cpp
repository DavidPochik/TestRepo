...
static Real g_a, delta_np, mcsq, sigma_0, tau_v, tau_epsilon, rho_epsilon, lmax;
// Optical depth
Real global_tau;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  lmax        = pin->GetReal("problem","MaxIteration");
...
//  Real rho_new = rho_old;
  Real rho_old, rho_new;
  AthenaArray<Real>& my_data = pmb->pmy_mesh->ruser_mesh_data[4];
//  Real tau = my_data(0);
  Real tau = global_tau;
  if (time != 0.0) {
    if ((tau > tau_v && tau < tau_v * (1.0 + tau_epsilon)) || (tau > tau_v * (1.0 - tau_epsilon) && tau < tau_v)){
      rho_new = prim(IDN,ks,js,is);
//      std::cout << "rho_new (converged) = " << rho_new << " g/cm^3" << std::endl;
...
//      std::cout << "too big" << std::endl;
      ...
      rho_new = rho_old * (1.0 - rho_epsilon);
      prim(IDN,ks,js,is) = rho_new;
//      std::cout << "rho_old (too big) = " << rho_old << " g/cm^3" << std::endl;
//      std::cout << "rho_new (too big) = " << rho_new << " g/cm^3" << std::endl;
      //rho_new = rho_old * (1.0 - rho_epsilon);
      //rho_old = rho_new;
    } else {
    ...
    std::cout << "t = 0, using default rho" << std::endl;
    rho_new = prim(IDN,ks,js,is);
  }
