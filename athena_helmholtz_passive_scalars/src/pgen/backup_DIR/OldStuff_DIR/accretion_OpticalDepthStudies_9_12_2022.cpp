...
// Optical Depth Quantities: axial vector coupling constant, neutron-proton mass difference, electron rest-mass energy,
...
// Setting neutrino optical depth to 2/3 (or at least im trying to)
  ...
  for (int l=1; l<=lmax; ++l){
    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<ie; ++i) {
          ...
          Real rho_ip1 = prim(IDN,k,j,i+1);
          Real rho_i   = prim(IDN,k,j,i);
          Real r_ip1   = pco->x1v(i+1);
          Real r_i     = pco->x1v(i);
          Real ye_ip1  = pmb->pscalars->r(ye_index,k,j,i+1);
          Real ye_i    = pmb->pscalars->r(ye_index,k,j,i);
          Real Chi_ip1 = (1.0 - ye_ip1);
          Real Chi_i   = (1.0 - ye_i);
          Real deltaR  = r_ip1 - r_i;
          tauInt       += 0.5 * deltaR * sigma_nue_n * Na * ((Chi_ip1 * rho_ip1) + (Chi_i * rho_i));
        }
      }
    }
    std::cout << "tauInt      = " << tauInt << std::endl;
    if ((tauInt > tau_v && tauInt < tau_v * (1.0 + tau_epsilon)) || (tauInt > tau_v * (1.0 - tau_epsilon) && tauInt < tau_v)){
      rho_new = rho_old;
      std::cout << "rho_new = " << rho_new << " g/cm^3" << std::endl;
      break;
    } else if (tauInt < tau_v * (1.0 - tau_epsilon)) {
    ...
    if (l==lmax) {
      std::cout << "(accretion.cpp) Tau integration loop has reached maximum iterations (" << lmax << ")" << std::endl;
      std::cout << " " << std::endl;
    }
  }
