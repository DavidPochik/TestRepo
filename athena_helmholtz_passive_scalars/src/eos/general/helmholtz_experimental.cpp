...
Real EquationOfState::PresFromRhoT(Real rho, Real T, Real* r) {
  ...
  phelm->HelmLookupRhoT(rho * rho_unit_, T, ye, abar, EosData);
  LastTemp = T;
...
// Putting this here for HSEInnerX1 and HSEInnerX2 BC Functions (just to compile)
...
Real EquationOfState::PresFromRhoT(Real rho, Real T) {
  ...
  Real abar = fixed_abar;
  Real temp = LastTemp;
  phelm->HelmLookupRhoT(rho * rho_unit_, ye, abar, T, EosData);
  LastTemp = T;
  return EosData(2) * inv_egas_unit_;
}
#if 0
...
void EquationOfState::InitEosConstants(ParameterInput *pin) {
//  std::cout << "ptable = " << ptable << std::endl;
//  std::cout << "phelm = " << phelm << std::endl;
