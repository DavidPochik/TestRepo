...
static Real r_0, inv_r2, rho_0, rho_f, v_f, p_f, mu, Ye, Na, p_0, p0, Tc, Th, tau, T_0, T_eq, Ye_eq, dpdd_0, B_0, alpha, T_floor, Mdot, Factor;
...
static Real tg, yg, dtg, dyg, toldNR, moddNR, maxcdNR, epsdNR, qdfactor, ydfactor;
...
void Double_NR(Real Arr[]) {
  ...
  for(int i=0; i<=maxcdNR; i++){
    ...
    // Set scale terms
    Real H0         = qdotScheck_H(Y0,r_0);
    Real H0_dT      = qdotScheck_H(Y0,r_0);
    Real H0_dY      = qdotScheck_H(Y0+deltaY0,r_0);
    Real Source0    = YeSource(T0*MeV,Y0,x_0,rho_0);
    Real Source0_dT = YeSource((T0+deltaT0)*MeV,Y0,x_0,rho_0);
    Real Source0_dY = YeSource(T0*MeV,Y0+deltaY0,x_0,rho_0);

    // Set 'original' functions
    Real qdot    = (qdotScheck_H(Y0,r_0) - qdotScheck_C(T0,Y0,rho_0,r_0));
    Real Yedot   = (YeSource(T0*MeV,Y0,x_0,rho_0) - YeSink(T0*MeV,Y0,x_0,rho_0));

    // Define functions with differentials
    Real dqdot_T  = (qdotScheck_H(Y0,r_0) - qdotScheck_C(T0+deltaT0,Y0,rho_0,r_0));
    Real dqdot_Y  = (qdotScheck_H(Y0+deltaY0,r_0) - qdotScheck_C(T0,Y0+deltaY0,rho_0,r_0));
    Real dYedot_T = (YeSource((T0+deltaT0)*MeV,Y0,x_0,rho_0) - YeSink((T0+deltaT0)*MeV,Y0,x_0,rho_0));
    Real dYedot_Y = (YeSource(T0*MeV,Y0+deltaY0,x_0,rho_0) - YeSink(T0*MeV,Y0+deltaY0,x_0,rho_0));
    ...
    // Define derivatives
    Real dqdotdT = (f_dT - f) / (dT*MeV);
    Real dqdotdY = (f_dY - f) / (dYe);
    Real dYdotdT = (g_dT - g) / (dT*MeV);
    ...
    //           A          *   x    =   b         //
    //                                             //
    // [ dqdotdT  dqdotdY ]   [ dT ]   [ dqdot  ]  //
    // [                  ] * [    ] = [        ]  //
    // [ dYdotdT  dYdotdY ]   [ dY ]   [ dYedot ]  //
    //                                             //
    //                          x    = A^-1 * b    //
    /////////////////////////////////////////////////
    ...
    Real delT   = (f*Ainv00 + g*Ainv01) / MeV;
    Real delYe  = f*Ainv10 + g*Ainv11;

    // Determine new T and Ye values
    Real T1 = T0 - delT;
    Real Y1 = Y0 - delYe;
...
//        std::cout << "delT  = " << delT << " K" << std::endl;
//        std::cout << "delYe = " << delYe << std::endl;
//        std::cout << "qdot  = " << qdot << "*10^21 erg/g/s" << std::endl;
//        std::cout << "Yedot = " << Yedot << "*10^2 s^-1" << std::endl;
//        std::cout << "|T1-T0|/|T1| = " << fabs(T1-T0)/fabs(T1) << std::endl;
//        std::cout << "|Y1-Y0|/|Y1| = " << fabs(Y1-Y0)/fabs(Y1) << std::endl;
//        std::cout << " " << std::endl;
        Arr[0] = T1;
        Arr[1] = Y1;
//        std::cout << "Success (Double_NR)" << std::endl;
        break;
    } else {
//      std::cout << "i            = " << i << std::endl;
//      std::cout << "T0           = " << T0 << " K" << std::endl;
...
//      std::cout << " " << std::endl;
    ...
    if(i==maxcdNR) {
      std::cout << "Maximum iterations reached (Douuble_NR)" << std::endl;
      Arr[0] = T1;
      Arr[1] = Y1;
    }
  }
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  qdfactor = pin->GetReal("problem","qdot_factor");
  ydfactor = pin->GetReal("problem","ydot_factor");
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin) {
  ...
  // Since these calculations don't vary in radius, I can perform them here.
  Real Arr[2];
  Double_NR(Arr);
  T_eq = Arr[0];
  Ye_eq = Arr[1];
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        efrac = &Ye_i;
        Real temp = pmb->peos->TFromRhoP(rho, p, efrac) * 8.6173e-11; //T in MeV
...
//        pmb->pscalars->r(ye_index,k,j,is-i) = pmb->pscalars->r(ye_index,k,j,is);
        ...
        if (NON_BAROTROPIC_EOS)
          prim(IPR,k,j,is-i) = p_0; //pmb->peos->PresFromRhoT(rho_0,T_eq,Ye_eq_ptr);
      }
    }
  }
...
//        std::cout << "efrac (UserWorkBeforeOutput) = " << phydro->u(IYE,k,j,i)/phydro->u(IDN,k,j,i) << std::endl;
        Real temp=peos->TFromRhoP(phydro->w(IDN,k,j,i),phydro->w(IPR,k,j,i),efrac);
