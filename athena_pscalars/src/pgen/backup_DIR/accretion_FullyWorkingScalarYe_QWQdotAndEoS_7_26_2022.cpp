...
//        }
        ...
        Real qdot = qdotQW(temp, x, Ye_i, time);
  ...
  for (int k=ks; k<=ke; ++k) {
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=1; i<=ngh; ++i) {
        ...
        pmb->pscalars->r(ye_index,k,j,is-i) = pmb->pscalars->r(ye_index,k,j,2);
...
//        Real yef  = Ye_f(temp, pscalars->r(ye_index,k,j,i), x);
       ...
       // user_out_var(2,k,j,i) = yef;//std::sqrt(peos->AsqFromRhoP(phydro->w(IDN,k,j,i),
                                //                            phydro->w(IPR,k,j,i),efrac));
        user_out_var(2,k,j,i) = eta;
      }
    }
  }
}
