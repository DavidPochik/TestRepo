  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        if(i==pmb->is){
          Real temp = zeroQ_temp(ye_0,rho_0,r_0) * 8.6173e-11; //T in MeV
          Real eta = QWEta(rho_0,temp/8.6173e-11,ye_0);
          Real vdYedr = YeSourceSink(temp,ye_0,0,eta);
          Real dYe = dt*vdYedr;
          cons_scalar(ye_index,k,j,i) += dYe*rho_0;
          Real qdot = qdotScheck_H(ye_0,r_0) - qdotScheck_C(temp/8.6173e-11,ye_0,rho_0,r_0); // erg/s/g units
          Real de = dt * rho_0 * qdot; // * 1.6022e-6; //removed tanh
          cons(IEN,k,j,i) += de;
          std::cout << "1" << std::endl;
        } else {
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      ...
      for(int i=is; i<=ie; i++) {
        ...
        efrac = &pscalars->r(ye_index,k,j,i);
        if(i==is){
          Real temp=zeroQ_temp(ye_0,rho_0,r_0);
          user_out_var(0,k,j,i) = temp;
          Real r = r_0;
          Real x=std::pow((1.0-(r_0*r_0)/(r*r)),0.5);
          Real eta = QWEta(rho_0,temp,ye_0);
          user_out_var(1,k,j,i) = qdotScheck_H(ye_0,r_0) - qdotScheck_C(temp,ye_0,rho_0,r_0);
