...
//  T_0    = zeroQ_temp(0.0, ye_0);
  ...
  p_0    = peos->PresFromRhoT(rho_0, T_0, Ye0);
...
//        Real p_A   = prim(IPR,k,j,is);
//        Real Ye_A  = pmb->pscalars->r(ye_index,k,j,is);
//        Real dpdd = pmb->peos->AsqFromRhoP(Rho_A, p_A, Ye_A);
...
//        std::cout << "v_A = " << prim(IVX,k,j,is) << std::endl;
//        std::cout << "v_G = " << prim(IVX,k,j,is-i) << std::endl;;
...
//	Real ye=pmb->pscalars->r(ye_index,k,j,is);
//        Real* ye_ptr=&ye;
//	Real rho=prim(IDN,k,j,is);
//	Real R=pco->x1v(is);
//        std::cout << "--------------INFLOWINNERX1--------------" << std::endl;
//	Real T_eq = zeroQ_temp(ye,rho,R);
//	std::cout << "ye      = " << ye << std::endl;
...
//          prim(IPR,k,j,is-i) = pmb->peos->PresFromRhoT(rho,T_eq,ye_ptr);
//          std::cout << "P = " << prim(IPR,k,j,is-i) << " erg/cm^3" << std::endl;
      }
    }
  }
