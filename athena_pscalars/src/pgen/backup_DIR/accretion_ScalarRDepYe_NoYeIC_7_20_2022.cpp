...
Real YeSourceSink(Real temp, Real ye, Real x) {
  ...
  Real lambda_nue_n  = std::pow(hbar*c,2.0)*(1.0+3.0*alpha*alpha)/(2*PI*PI)*G_F*G_F*L_nue/(r_0*r_0)
                      *(e_nue+2.0*Delta+1.2*Delta*Delta/e_nue)*(1.0-x); // s^-1 units
  Real lambda_nueb_p = std::pow(hbar*c,2.0)*(1.0+3.0*alpha*alpha)/(2*PI*PI)*G_F*G_F*L_nueb/(r_0*r_0)
                      *(e_nueb+2.0*Delta+1.2*Delta*Delta/e_nueb)*(1.0-x); // s^-1 units

  // Reverse reaction rates
  Real lambda_ele_p = 0.448*std::pow(temp,5.0); // s^-1 units (e- p)
  Real lambda_pos_n = lambda_ele_p; // s^-1 units (e+ n)
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        Real vdYedr = YeSourceSink(temp,Ye_i,x);
        Real dYe = dt*vdYedr;
        cons_scalar(ye_index,k,j,i) += dYe*rho;
        Real elecfrac = cons_scalar(ye_index,k,j,i)/rho;
