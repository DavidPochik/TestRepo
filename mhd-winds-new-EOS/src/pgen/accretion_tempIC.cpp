...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin) {
  ...
  dpdd_0 = peos->AsqFromRhoP(rho_0,p_0);
...
//          Real mdot = 0.3;
//          if(r<1.7e7 and rho_0>1.2e9) {
////            Real qp= std::pow(r_0/r,20.0);
////            rho=rho_0*qp;
////            Real mdot= 4.0*3.14*r*r*rho_in(index)*v_in(index);
//            Real v0 = -1.5e7;
//            v = v0 * std::pow(r/r0,2.5);
//            Real absv = std::abs(v);
//            rho = mdot * (2e33) / (4 * 3.14 * r * r * absv);
////            std::cout<< "rho(r<r_shock) = " << rho << "\n";
////            std::cout<< "v(r<r_shock)   = " << v << "\n";
////            std::cout<< "r              = " << r << "\n";
////            std::cout<< "r0             = " << r0 << "\n";
////            std::cout<< "i              = " << i << "\n";
////            printf("\n");
...
////            std::cout<< "v   = " << v << "\n";
////            std::cout<< "rho = " << rho << "\n";
//            Real vps = -1.0 * std::pow(2.0*mu/r,0.5);
////            std::cout<< "countps(before) = " << countps << "\n";
//            if(countps<1) {
//               fact = vps / v;
//               vfactor = fact / 7;
////               printf("howdy\n");
//            }
//            countps++;
////            std::cout<< "countps(after)  = " << countps << "\n";
//            v = -1.0 * std::pow(2.0*mu/r,0.5) / vfactor;   //2.13226;
////            std::cout<< "vfactor        = " << vfactor << "\n";
////            std::cout<< "v(r>r_shock)   = " << v << "\n";
////            std::cout<< "i              = " << i << "\n";
////            std::cout<< "mu             = " << mu << "\n";
////            std::cout<< "r              = " << r << "\n";
...
//          }
          ...
          if (NON_BAROTROPIC_EOS) {
            if (GENERAL_EOS) {
              Real pressure = peos->PresFromRhoT(rho, temp);
              phydro->u(IEN,k,j,i) = peos->EgasFromRhoP(rho, pressure);
            }
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        Real temp = pmb->peos->TFromRhoP(rho, p) * 8.6173e-11; //T in MeV
...
void InflowOuterX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
 ...
 // Real re = pco->x1v(ie);
  for (int k=ks; k<=ke; ++k) {
   ...
   //     Real rgh = pco->x1v(ie+i);
   //    Real ratio = re / rgh;
   //     Real v = prim(IVX,k,j,ie) * SQR(pco->x1v(ie) / pco->x1v(ie+i));
        prim(IDN,k,j,ie+i) = rho_f;
//	std::cout<< "rho_f (InflowOuterX1)              = " << rho_f << "\n";
//	std::cout<< "prim(IDN,k,j,ie+i) (InflowOuterX1) = " << prim(IDN,k,j,ie+i) << "\n";
   //     prim(IVX,k,j,ie+i) = std::max(v, 0.0);
        prim(IVX,k,j,ie+i) = v_f;
//	std::cout<< "hello";
	Real tempc         = pmb->peos->TFromRhoP(prim(IDN,k,j,ie),prim(IPR,k,j,ie));
	prim(IPR,k,j,ie+i) = pmb->peos->PresFromRhoT(prim(IDN,k,j,ie+i),tempc);
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin) {
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      ...
      for(int i=is; i<=ie; i++) {
        Real temp=peos->TFromRhoP(phydro->w(IDN,k,j,i),phydro->w(IPR,k,j,i));
