...
Real PorE(Real rho, Real T, int index) {
  Real vol = mn/rho;
  Real T4 = T*T*T*T;
...
void QWData(Real rho, Real T, AthenaArray<Real> &OutData) {
  ...
  Real der_eta = (std::pow(2.0,third)/eta_den_const)*(dterm_dt/a - term*da_dt/(a*a))
                 - (2.0*std::pow(3.0,third)/eta_den_const)*(da_dt/term -a*dterm_dt/(term*term));
...
void TempInvert(Real rho, Real GuessTemp, Real var, const int index,
  ...
  if (index == EOS::iP) {
    ...
    Real v1= PorE(rho,tt1,index);
    Real v3= PorE(rho,GuessTemp,index);
...
void EquationOfState::InitEosConstants(ParameterInput* pin) {
  ...
  if ((Globals::my_rank == 0) && (pmy_block_->lid == 0)) {
    std::cout<<Ye<<" Ye\n";
    std::cout<<"Using updated EOS QW NEW 3 (4/1/22) \n";
  }
  #ifdef MYDEBUG
    Real rho, temp;
