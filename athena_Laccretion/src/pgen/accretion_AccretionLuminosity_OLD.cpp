...
Real YeTejas_Lnu(Real temp, Real ye, Real x, Real Rho, Real Lnu_r) {
  // Returns Sources for Yedot=vdYe/dr in s^-1 units
  // Temperature argument is in MeV
  // Define constants
  Real alpha    = 1.26;                                // Coupling Coefficient
  ...
  Real erg2MeV  = 6.24151e5;                           // convert erg to MeV
  //Luminosity and epsilon
  Real lnu_y      = Lnu_r;                    // ASSUMES Lnue and Lnueb are EQUAL!
  Real epsnu_y    = eps_nue;
  Real lnubar_y   = Lnu_r;
  Real epsnubar_y = eps_nueb;
...
Real Heating_QW_Modified_Lnu(Real x, Real Ye, Real Lnu_r) {
  // returns heating term from QW 1996
  // Assumes L_nue = L_nueb!
  Real sigma_0   = 1.76e-44;                                     // Weak interaction cross section in cm^2, taken from Scheck et al 2006
  Real alpha     = 1.26;                                         // Axial coupling coefficient used in QW 1996
  Real mn        = 1.0 / Na;                                     // Baryon mass in g
  Real E_e       = 0.511;                                        // Electrn energy in MeV
...
//                   (8.0 * PI * mn * std::pow(E_e,2.0) * 1.0e12); // Heating prefactor in erg s^-1 g^-1
  ...
  // Heating term in erg s^-1 g^-1, L_nue and L_nueb are in 10^51 erg/s units
 // Real H = prefactor * F * (Xn * L_nue * std::pow(kbT_nue,2.0) + Xp * L_nueb * std::pow(kbT_nueb,2.0)) * (1.0 - x) / std::pow(r_0/1.0e6,2.0);
  Real H = MeV2erg * prefactor * F * (Xn * Lnu_r * std::pow(kbT_nue,2.0) + Xp * Lnu_r * std::pow(kbT_nueb,2.0)) * (1.0 - x) / std::pow(r_0,2.0); // erg/s/g
  return H;
}
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (use_IC_file) {
    ...
    ruser_mesh_data[10].NewAthenaArray(rows);
//    ruser_mesh_data[11].NewAthenaArray(1);
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  if (use_IC_file) {
    ...
    // accretion luminosity array
    AthenaArray<Real>& Lacc = pmy_mesh->ruser_mesh_data[10];
    ...
    for (int k=ks; k<=ke; k++) {
      ...
      for (int j=js; j<=je; j++) {
        ...
        Real localaccsum = 0.0;
        for (int i=is; i<=ie; i++) {
          Real r   = pcoord->x1v(i);
          Real drf = pcoord->x1f(i+1) - pcoord->x1f(i);
          Real r0 = 5e6;
          Real mn = 1.6749286e-24;                // baryon mass in g
          Real rho, v, temp, ye;
          ...
          if (r>5.0e6 && temp_MeV<1.0) {
            X_n = X_nucleon(rho, temp_MeV);
          }
          Real x        = std::sqrt(1.0-(r_0*r_0)/(r*r));
          Real ExpSupp  = std::exp(-1.0 * rho / rho_0);
          Real vdYedr   = ExpSupp * std::min(1.0,X_n) * YeTejas(temp_MeV, ye, x, rho);
          Real qdot     = ExpSupp * std::min(1.0,X_n) * (Heating_QW_Modified(x, ye) - Cooling_QW_Modified(rho, temp_MeV, ye)); // erg/s/g
          localaccsum += -1.0 * 4.0 * PI * std::pow(r,2.0) * rho * qdot * drf;
          Lacc(i+process_Rank*(ie-is+1)) = localaccsum;

          if(size_Of_Cluster>1){
            if(i==ie){
              for(int q = 0; q < size_Of_Cluster-1; q++){
                if(q == process_Rank){
                  if(q!=0){
           //         printf("receive sum term from q=%d on q=%d\n", q-1, q);
                    MPI_Recv(&message_Item, 1, MPI_DOUBLE, q-1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
           //         printf("receive command executed, sum_term=%.4e\n", sum_term);
                    for (int l=is; l<=ie; l++) {
                      Lacc(l+process_Rank*(ie-is+1)) += message_Item;
                    }
                    localaccsum += message_Item;
                    message_Item = localaccsum;
                  } else {
                    message_Item = localaccsum;
                  }
                  if(q < size_Of_Cluster - 1) {
           //         printf("send sum_term=%.4e from q=%d to q=%d\n", sum_term, q, q+1);
                    MPI_Send(&message_Item, 1, MPI_DOUBLE, q+1, 1, MPI_COMM_WORLD);
                //    delete &sum_term;
      ...
      //    if(i==ie){
      //      for(int q = 0; q < size_Of_Cluster; q++){
      //        if(q == process_Rank){
      //          if(q==0){
//    //              printf("q = %d, localaccsum   = %.4e\n", q, localaccsum);
//    //              printf("Lacc at edge of block = %.4e\n", Lacc(i+process_Rank*(ie-is+1)));
      //            message_Item = localaccsum;
      //            // precaution for serial calculation
      //            if(q < size_Of_Cluster - 1){
      //              MPI_Send(&message_Item, 1, MPI_DOUBLE, q+1, 1, MPI_COMM_WORLD);
      //          //    delete &message_Item;
      //            } else {
      ...
      //            for (int l=is; l<=ie; l++) {
      //           //   if(k==ie){
      //          //      printf("Lacc at edge of block before addition = %.4e\n", Lacc(k+process_Rank*(ie-is+1)));
      //          //    }
      //              Lacc(k+process_Rank*(ie-is+1)) += message_Item;
      //           //   if(k==ie){
      //           //     printf("Lacc at edge of block after addition  = %.4e\n", Lacc(k+process_Rank*(ie-is+1)));
      //           //   }
      //            }
      //         //   printf("q = %d, localaccsum (before adding new sum) = %.4e\n", q, localaccsum);
      //            localaccsum  += message_Item;
      //         //   printf("q = %d, localaccsum (after adding new sum)  = %.4e\n", q, localaccsum);
      //         //   printf("oldsum value = %.4e\n", message_Item);
      //            message_Item = localaccsum;
      //          //  printf("newsum value (should match new localaccsum) = %.4e\n", message_Item);
      //          //  std::cout << " " << std::endl;
      //            if(q < size_Of_Cluster - 1){
      //              MPI_Send(&message_Item, 1, MPI_DOUBLE, q+1, 1, MPI_COMM_WORLD);
      //           //   delete &message_Item;
...
void heat_cool(MeshBlock *pmb, const Real time, const Real dt,
  ...
  MPI_Comm_size(MPI_COMM_WORLD, &size_Of_Cluster);
  MPI_Comm_rank(MPI_COMM_WORLD, &process_Rank);
  AthenaArray<Real>& Lacc{pmb->pmy_mesh->ruser_mesh_data[10]};
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      ...
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        Real drf      = pmb->pcoord->x1f(i+1) - pmb->pcoord->x1f(i);
...
//        std::cout << "(heat_cool) Lacc(" << i+process_Rank*(pmb->ie-pmb->is+1) << ") = " << Lacc(i+process_Rank*(pmb->ie-pmb->is+1)) << " erg/g" << std::endl;
//        std::cout << "(heat_cool) r(" << i << ") = " << r << "\n" << std::endl;
        Real r_scalar[nscalar_size];
        Real s_scalar[nscalar_size];
        for (int ns=0; ns<nscalar_size; ns++) {
...
//        if (temp < 1.0) {
        ...
        Real x         = std::sqrt(1.0-(r_0*r_0)/(r*r));
        Real ExpSupp   = std::exp(-1.0 * rho / rho_0);
        Real Lcore     = L_nue; // ASSUMES Lnue and Lnueb are EQUAL
        Real LnuTotal  = Lcore + Lacc(i+process_Rank*(pmb->ie-pmb->is+1)) / 1.0e51; // 10^51 erg/s units
...
//        Real vdYedr = std::min(1.0, X_n) * (YeSource_QW_Modified(rho,temp,Ye_i,x,r) - YeSink_QW_Modified(rho,temp,Ye_i,x,r));
        Real vdYedr = ExpSupp * std::min(1.0,X_n) * YeTejas_Lnu(temp, Ye_i, x, rho, LnuTotal);
//        Real qdot   = std::min(1.0, X_n) * (qdotScheck_H(Ye_i,r) - qdotScheck_C(temp/kbol_MeV,Ye_i,rho,r)); // erg/s/g units
     //   Real qdot = std::min(1.0,X_n) * (Heating_QW_Modified(x, Ye_i) - Cooling_QW_Modified(rho, temp, Ye_i));
      //  Real qdot = qdotQW(temp, x, Ye_i, time) / erg2MeV;
        Real qdot   = ExpSupp * std::min(1.0,X_n) * (Heating_QW_Modified_Lnu(x, Ye_i, LnuTotal) - Cooling_QW_Modified(rho, temp, Ye_i)); // erg/s/g
//        std::cout << "qdot = " << qdot << " erg/s/g" << std::endl;
...
//        }
        ...
        // Update accretion luminosity
        localaccsum += -1.0 * 4.0 * PI * std::pow(r,2.0) * rho * qdot * drf;
        Lacc(i+process_Rank*(pmb->ie-pmb->is+1)) = localaccsum;
        if(size_Of_Cluster>1){
          if(i==pmb->ie){
            for(int q = 0; q < size_Of_Cluster-1; q++){
              if(q == process_Rank){
                if(q!=0){
         //         printf("receive sum term from q=%d on q=%d\n", q-1, q);
                  MPI_Recv(&sum_term, 1, MPI_DOUBLE, q-1, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
         //         printf("receive command executed, sum_term=%.4e\n", sum_term);
                  for (int l=pmb->is; l<=pmb->ie; l++) {
                    Lacc(l+process_Rank*(pmb->ie-pmb->is+1)) += sum_term;
                  }
                  localaccsum += sum_term;
                  sum_term = localaccsum;
                } else {
                  sum_term = localaccsum;
                }
                if(q < size_Of_Cluster - 1) {
         //         printf("send sum_term=%.4e from q=%d to q=%d\n", sum_term, q, q+1);
                  MPI_Send(&sum_term, 1, MPI_DOUBLE, q+1, 2, MPI_COMM_WORLD);
              //    delete &sum_term;
         //         printf("send command executed\n");
                } else {
              //    delete &sum_term;
...
//        std::cout << "P_GhostZ (Output step) = " << P_GhostZ << " erg/cm^3" << std::endl;
        ...
        Real Lcore    = L_nue;
        Real LnuTotal = Lcore + Lacc(i+process_Rank*(ie-is+1)) / 1.0e51; // 10^51 erg/s units
        Real temp_MeV = temp * kbol_MeV;
        Real t        = pmy_mesh->time;
        Real X_n      = 1.0; //Xn(rho, temp);
        if (r>5.0e6 && temp_MeV < 1.0) {
...
//        user_out_var(1,k,j,i) = qdotQW(temp_MeV, x, ye, t) / erg2MeV;
        user_out_var(1,k,j,i) = ExpSupp * std::min(1.0, X_n) * (Heating_QW_Modified_Lnu(x, ye, LnuTotal) - Cooling_QW_Modified(rho, temp_MeV, ye));
        // Sound Speed
...
//        std::cout << "rho (u)  = " << phydro->u(IDN,k,j,i) << " g/cm^3" << std::endl;
        ...
        user_out_var(15,k,j,i) = Lacc(i+process_Rank*(ie-is+1)) / 1.0e51; //sigma_0 * ((1.0 + 3.0 * std::pow(g_a,2)) / (4.0)) * std::pow(((eps_nue + delta_np) / (mcsq)),2); // Electron neutrino x-section
      }
    }
  }
}
