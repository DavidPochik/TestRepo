          ...
          if(size_Of_Cluster>1){
            if(i==ie){
              for(int q = 0; q < size_Of_Cluster; q++){
                if(q == process_Rank){
                  if(q!=0){ 
                    MPI_Recv(&sum_term, 1, MPI_DOUBLE, q-1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
                    for (int l=is; l<=ie; l++) {
                      Lacc(l+process_Rank*(ie-is+1)) += sum_term;
                    }
                  ...
                  if(q < size_Of_Cluster - 1) {
                    MPI_Send(&sum_term, 1, MPI_DOUBLE, q+1, 1, MPI_COMM_WORLD);
                  }
                }
...
//        std::cout << "rho (u)  = " << phydro->u(IDN,k,j,i) << " g/cm^3" << std::endl;
        ...
        user_out_var(15,k,j,i) = LnuTotal; //10^51 erg/s //sigma_0 * ((1.0 + 3.0 * std::pow(g_a,2)) / (4.0)) * std::pow(((eps_nue + delta_np) / (mcsq)),2); // Electron neutrino x-section
      }
    }
  } 
}
