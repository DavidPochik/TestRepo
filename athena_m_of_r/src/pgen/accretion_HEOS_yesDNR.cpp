...
// Problem quantities: gravitational mass, gravitational constant, PNS mass, inner density, outer density, inner temperature, outer velocity, outer pressure, mass accretion rate, inner radius,
...
static Real mu, G, M_PNS, rho_0, rho_f, T_0, v_f, p_f, Mdot, r_0, inv_r2, Na, Tc, B_0, alpha, t_rho0_0;
...
static int IDT1, IDT2, IDT3, IDT4, IDT5, IDT6, IDT7, IDT8;
...
//  std::cout << "(YeSourceQW) Source = " << out << " s^-1" << std::endl;
  return out;
}
...
//  std::cout << "(YeSinkQW) lambda_nue_n  = " << lambda_nue_n  << " s^-1" << std::endl;
//  std::cout << "(YeSinkQW) lambda_pos_n  = " << lambda_pos_n  << " s^-1" << std::endl;
//  std::cout << "(YeSinkQW) lambda_nueb_p = " << lambda_nueb_p << " s^-1" << std::endl;
//  std::cout << "(YeSinkQW) lambda_ele_p  = " << lambda_ele_p  << " s^-1" << std::endl;
//  std::cout << " " << std::endl;

//  std::cout << "(YeSinkQW) Sink = " << out << " s^-1" << std::endl;
//  std::cout << "(YeSinkQW) lambda_nue_n  = " << lambda_nue_n  << " s^-1" << std::endl;
//  std::cout << "(YeSinkQW) lambda_nueb_p = " << lambda_nueb_p << " s^-1" << std::endl;
//  std::cout << "(YeSinkQW) lambda_ele_p  = " << lambda_ele_p  << " s^-1" << std::endl;
//  std::cout << "(YeSinkQW) lambda_pos_n  = " << lambda_pos_n  << " s^-1" << std::endl;
...
//  Real Delta = 0.0;
  ...
  Real n_e   = 8.0 * PI / std::pow(hbar * 2.0 * PI * c,3) * std::pow(temp,3) * fermi_approx(2,eta_e);        // electron number density, cm^-3
  Real n_pos = 8.0 * PI / std::pow(hbar * 2.0 * PI * c,3) * std::pow(temp,3) * fermi_approx(2,-1.0 * eta_e); // positron number density, cm^-3
  Real n_p   = n_e;                                                                                          // proton number density, cm^-3

  // Neutrino energy moments given by D.42, assume eta_nu = 0
  Real Tnue    = eps_nue  / (kbol_MeV)       * (fermi_approx(2,0) / fermi_approx(3,0));  // K
  Real EpsNue2 = std::pow(Tnue * kbol_erg,2) * (fermi_approx(4,0) / fermi_approx(2,0));  // erg^2
  Real EpsNue1 = Tnue * kbol_erg             * (fermi_approx(3,0) / fermi_approx(2,0));  // erg

  // Positron energy moments given by D.52
  Real EpsPositron2 = std::pow(temp * MeV2erg,2) * (fermi_approx(4,-1.0 * eta_e) / fermi_approx(2,-1.0*eta_e));  // erg^2
  Real EpsPositron1 = temp * MeV2erg             * (fermi_approx(3,-1.0 * eta_e) / fermi_approx(2,-1.0*eta_e));  // erg
...
//  std::cout << "(YeSinkHEOS) eta_e = " << eta_e << std::endl;
//  Real eta_e = 0.0;
//  Real Delta = 0.0;
  // Composition densities (n_e and n_pos are given by equation D.51, which has a typo and is missing temperature)
  Real nb    = rho / mn;                                                                                   // baryon number density, cm^-3
  ...
  // Neutrino energy moments given by D.42, assume eta_nu = 0
  Real Tnue    = eps_nue  / (kbol_MeV)       * (fermi_approx(2,0) / fermi_approx(3,0));  // K
//  std::cout << "(YeSinkHEOS) Tnue  = " << Tnue * kbol_MeV << " MeV" << std::endl;
  Real EpsNue2 = std::pow(Tnue * kbol_erg,2) * (fermi_approx(4,0) / fermi_approx(2,0));  // erg^2
  Real EpsNue1 = Tnue * kbol_erg             * (fermi_approx(3,0) / fermi_approx(2,0));  // erg

  // Neutrino energy moments given by D.43, assume eta_nu = 0
  Real Tnueb        = eps_nueb  / (kbol_MeV)       * (fermi_approx(2,0) / fermi_approx(3,0));                      // K
//  std::cout << "(YeSinkHEOS) Tnueb = " << Tnueb * kbol_MeV << " MeV" << std::endl;
//  std::cout << "(YeSinkHEOS) T     = " << temp << " MeV" << std::endl;
//  std::cout << "(YeSinkHEOS) Delta/k_b Tnue  = " << Delta / (kbol_MeV * Tnue ) << std::endl;
//  std::cout << "(YeSinkHEOS) Delta/k_b Tnueb = " << Delta / (kbol_MeV * Tnueb ) << std::endl;
//  std::cout << "(YeSinkHEOS) Delta/k_b T     = " << Delta / (temp) << std::endl;

/////// REGULAR ENERGY MOMENTS

  Real EpsNueb2Star = std::pow(Tnueb * kbol_erg,2) * (fermi_approx(4,-1.0 * Delta / Tnueb) / fermi_approx(2,0));  // erg^2
  Real EpsNueb1Star = Tnueb * kbol_erg             * (fermi_approx(3,-1.0 * Delta / Tnueb) / fermi_approx(2,0));  // erg
  Real EpsNueb0Star = 1.0                          * (fermi_approx(2,-1.0 * Delta / Tnueb) / fermi_approx(2,0));  // unitless
  Real EpsNueb1     = Tnueb * kbol_erg             * (fermi_approx(3,0) / fermi_approx(2,0));                      // erg

/////// MODIFIED ENERGY MOMENTS

//  Real EpsNueb2Star = std::pow(Tnueb * kbol_erg,2) * (fermi_approx(4,0.0) / fermi_approx(2,0.0));  // erg^2
//  Real EpsNueb1Star = Tnueb * kbol_erg             * (fermi_approx(3,0.0) / fermi_approx(2,0.0));  // erg
//  Real EpsNueb0Star = 1.0                          * (fermi_approx(2,0.0) / fermi_approx(2,0.0));  // unitless
//  Real EpsNueb1     = Tnueb * kbol_erg             * (fermi_approx(3,0.0) / fermi_approx(2,0.0));                      // erg
  ...
  ////// REGULAR ENERGY MOMENTS
  Real EpsElectron2Star = std::pow(temp * MeV2erg,2) * (fermi_approx(4,eta_e - Delta/(temp)) / fermi_approx(2,eta_e));  // erg^2
  Real EpsElectron1Star = temp * MeV2erg             * (fermi_approx(3,eta_e - Delta/(temp)) / fermi_approx(2,eta_e));  // erg
  Real EpsElectron0Star = 1.0                        * (fermi_approx(2,eta_e - Delta/(temp)) / fermi_approx(2,eta_e));  // unitless

  ////// MODIFIED ENERGY MOMENTS
//  Real EpsElectron2Star = std::pow(temp * MeV2erg,2) * (fermi_approx(4,eta_e) / fermi_approx(2,eta_e));  // erg^2
//  Real EpsElectron1Star = temp * MeV2erg             * (fermi_approx(3,eta_e) / fermi_approx(2,eta_e));  // erg
//  Real EpsElectron0Star = 1.0                        * (fermi_approx(2,eta_e) / fermi_approx(2,eta_e));  // unitless
...
//  std::cout << "(YeSinkHEOS) lambda_nue_n  = " << 1.0/nb * lambda_nue_n  << " s^-1" << std::endl;
//  std::cout << "(YeSinkHEOS) lambda_pos_n  = " << 1.0/nb * lambda_pos_n  << " s^-1" << std::endl;
//  std::cout << "(YeSinkHEOS) lambda_nueb_p = " << 1.0/nb * lambda_nueb_p << " s^-1" << std::endl;
//  std::cout << "(YeSinkHEOS) lambda_ele_p  = " << 1.0/nb * lambda_ele_p  << " s^-1" << std::endl;
//  std::cout << " " << std::endl;
  // Sink term
...
//  Real Delta    = 0.0;
  ...
  Real Tnue         = eps_nue  / (kbol_MeV)        * (fermi_approx(2,0)                    / fermi_approx(3,0));  // K
  Real Tnueb        = eps_nueb / (kbol_MeV)        * (fermi_approx(2,0)                    / fermi_approx(3,0));  // K
  Real EpsNue2      = std::pow(Tnue  * kbol_MeV,2) * (fermi_approx(4,0)                    / fermi_approx(2,0));  // MeV^2
  Real EpsNueb2     = std::pow(Tnueb * kbol_MeV,2) * (fermi_approx(4,0)                    / fermi_approx(2,0));  // MeV^2
  Real EpsNueb2Star = std::pow(Tnueb * kbol_MeV,2) * (fermi_approx(4,-1.0 * Delta / Tnueb) / fermi_approx(2,0));  // MeV^2
  Real EpsNue1      = Tnue  * kbol_MeV             * (fermi_approx(3,0)                    / fermi_approx(2,0));  // MeV
  Real EpsNueb1     = Tnueb * kbol_MeV             * (fermi_approx(3,0)                    / fermi_approx(2,0));  // MeV
  Real EpsNueb1Star = Tnueb * kbol_MeV             * (fermi_approx(3,-1.0 * Delta / Tnueb) / fermi_approx(2,0));  // MeV
  Real EpsNueb0Star = 1.0                          * (fermi_approx(2,-1.0 * Delta / Tnueb) / fermi_approx(2,0));  // unitless
...
//  Real Delta    = 0.0;
  ...
  // Calculate Eta, send in T in K
  Real Eta = QWEta(Rho,temp/kbol_MeV,ye);
  std::cout << "(YeSourceQW) eta_e = " << Eta << std::endl;
...
// Ye source function for scalar Ye (Scheck et al. 2006)
Real YeSourceHEOS_nan(Real temp, Real ye, Real rho, Real r){
  // Returns YeDot_source in s^-1 units
  // Temperature argument is in MeV
  // Define constants
  Real alpha    = 1.26;                                                                                 // Coupling Coefficient
  ...
  std::cout << "(Source) fnu    = " << fnu    << std::endl;
  std::cout << "(Source) Tprime = " << Tprime << " cm^-1"       << std::endl;
  std::cout << "(Source) Terg   = " << Terg   << " erg"         << std::endl;
  std::cout << "(Source) tfact  = " << tfact  << " g^1/3 cm^-1" << std::endl;

  // Electron chemical potential
  Real eta_e    = std::pow(12.0 * PI,1.0/3.0) * (std::pow(PI * mn,1.0/3.0) * std::pow(tfact,2) -
                           PI * std::pow(Tprime,2) * mn * std::pow(12.0,1.0/3.0)) / (6.0 * std::pow(mn,2.0/3.0) * tfact * Tprime); // unitless

  std::cout << "(Source) eta_e = " << eta_e << std::endl;
  ...
  std::cout << "(Source) nb    = " << nb    << " cm^-3" << std::endl;
  std::cout << "(Source) n_e   = " << n_e   << " cm^-3" << std::endl;
  std::cout << "(Source) n_pos = " << n_pos << " cm^-3" << std::endl;
  std::cout << "(Source) n_p   = " << n_p   << " cm^-3" << std::endl;
  ...
  std::cout << "(Source) Tnue    = " << Tnue    << " K"     << std::endl;
  std::cout << "(Source) EpsNue2 = " << EpsNue2 << " erg^2" << std::endl;
  std::cout << "(Source) EpsNue1 = " << EpsNue1 << " erg"   << std::endl;

  // Positron energy moments given by D.52
  Real EpsPositron2 = std::pow(temp * MeV2erg,2) * (fermi_approx(4,-1.0 * eta_e) / fermi_approx(2,-1.0*eta_e));  // erg^2
  Real EpsPositron1 = temp * MeV2erg             * (fermi_approx(3,-1.0 * eta_e) / fermi_approx(2,-1.0*eta_e));  // erg

  std::cout << "(Source) EpsPositron2 = " << EpsPositron2 << " erg^2" << std::endl;
  std::cout << "(Source) EpsPositron1 = " << EpsPositron1 << " erg"   << std::endl;
  ...
  std::cout << "(Source) lambda_nue_n = " << lambda_nue_n << " cm^-3 s^-1" << std::endl;
  std::cout << "(Source) lambda_pos_n = " << lambda_pos_n << " cm^-3 s^-1" << std::endl;

  // Source term
  Real source = 1.0 / nb * (lambda_nue_n + lambda_pos_n); // s^-1

  std::cout << "(Source) source = " << source << " s^-1" << std::endl;
...
// Ye sink function for scalar Ye (Scheck et al. 2006)
Real YeSinkHEOS_nan(Real temp, Real ye, Real rho, Real r){
  // Returns Yedot_sink in s^-1 units
  // Temperature argument is in MeV
  // Define constants
  Real alpha    = 1.26;                                                                             // Coupling Coefficient
  ...
  std::cout << "(Sink) fnu    = " << fnu    << std::endl;
  std::cout << "(Sink) Tprime = " << Tprime << " cm^-1"       << std::endl;
  std::cout << "(Sink) Terg   = " << Terg   << " erg"         << std::endl;
  std::cout << "(Sink) tfact  = " << tfact  << " g^1/3 cm^-1" << std::endl;

  // Electron chemical potential
  Real eta_e    = std::pow(12.0 * PI,1.0/3.0) * (std::pow(PI * mn,1.0/3.0) * std::pow(tfact,2) -
                           PI * std::pow(Tprime,2) * mn * std::pow(12.0,1.0/3.0)) / (6.0 * std::pow(mn,2.0/3.0) * tfact * Tprime); // unitless

  std::cout << "(Sink) eta_e = " << eta_e << std::endl;
  ...
  std::cout << "(Sink) nb    = " << nb    << " cm^-3" << std::endl;
  std::cout << "(Sink) n_e   = " << n_e   << " cm^-3" << std::endl;
  std::cout << "(Sink) n_pos = " << n_pos << " cm^-3" << std::endl;
  std::cout << "(Sink) n_p   = " << n_p   << " cm^-3" << std::endl;
  ...
  std::cout << "(Sink) Tnue    = " << Tnue    << " K"     << std::endl;
  std::cout << "(Sink) EpsNue2 = " << EpsNue2 << " erg^2" << std::endl;
  std::cout << "(Sink) EpsNue1 = " << EpsNue1 << " erg"   << std::endl;
  ...
  std::cout << "(Sink) Tnueb        = " << Tnueb        << " K"     << std::endl;
  std::cout << "(Sink) EpsNueb2Star = " << EpsNueb2Star << " erg^2" << std::endl;
  std::cout << "(Sink) EpsNueb1Star = " << EpsNueb1Star << " erg"   << std::endl;
  std::cout << "(Sink) EpsNueb0Star = " << EpsNueb0Star             << std::endl;
  std::cout << "(Sink) EpsNueb1     = " << EpsNueb1                 << std::endl;

  // Positron energy moments given by D.52
  Real EpsPositron2 = std::pow(temp * MeV2erg,2) * (fermi_approx(4,-1.0 * eta_e) / fermi_approx(2,-1.0*eta_e));  // erg^2
  Real EpsPositron1 = temp * MeV2erg             * (fermi_approx(3,-1.0 * eta_e) / fermi_approx(2,-1.0*eta_e));  // erg

  std::cout << "(Sink) EpsPositron2         = " << EpsPositron2 << " erg^2" << std::endl;
  std::cout << "(Sink) EpsPositron1         = " << EpsPositron1 << " erg"   << std::endl;
  std::cout << "(Sink) fermi_approx(4,-eta) = " << fermi_approx(4,-1.0 * eta_e) << std::endl;
  std::cout << "(Sink) fermi_approx(3,-eta) = " << fermi_approx(3,-1.0 * eta_e) << std::endl;
  std::cout << "(Sink) fermi_approx(2,-eta) = " << fermi_approx(2,-1.0 * eta_e) << std::endl;
  std::cout << "(Sink) temp                 = " << temp << " MeV"               << std::endl;
  ...
  std::cout << "(Sink) EpsElectron2Star = " << EpsElectron2Star << " erg^2" << std::endl;
  std::cout << "(Sink) EpsElectron1Star = " << EpsElectron1Star << " erg"   << std::endl;
  std::cout << "(Sink) EpsElectron0Star = " << EpsElectron0Star     << std::endl;
  std::cout << "(Sink) fermi_approx(4,eta-Delta/T) = " << fermi_approx(4,eta_e - Delta / temp) << std::endl;
  std::cout << "(Sink) fermi_approx(3,eta-Delta/T) = " << fermi_approx(3,eta_e - Delta / temp) << std::endl;
  std::cout << "(Sink) fermi_approx(2,eta-Delta/T) = " << fermi_approx(2,eta_e - Delta / temp) << std::endl;
  std::cout << "(Sink) fermi_approx(2,eta)         = " << fermi_approx(2,eta_e)                << std::endl;
  ...
  std::cout << "(Sink) lambda_nue_n  = " << lambda_nue_n  << " cm^-3 s^-1" << std::endl;
  std::cout << "(Sink) lambda_pos_n  = " << lambda_pos_n  << " cm^-3 s^-1" << std::endl;
  std::cout << "(Sink) lambda_nueb_p = " << lambda_nueb_p << " cm^-3 s^-1" << std::endl;
  std::cout << "(Sink) lambda_ele_p  = " << lambda_ele_p  << " cm^-3 s^-1" << std::endl;

  // Sink term
  Real sink = 1.0 / nb * ye * (lambda_nue_n + lambda_pos_n + lambda_nueb_p + lambda_ele_p); // s^-1

  std::cout << "(Sink) sink = " << sink << " s^-1" << std::endl;
...
//  mu       = pin->GetReal("problem","GM");
  G        = pin->GetReal("problem","G_Newt");
  M_PNS    = pin->GetReal("problem","interior_mass");
  mu       = G * M_PNS;
  ...
  if (use_IC_file) {
    ...
    // Data structure for calculating optical depth
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        Real vdYedr = YeSourceHEOS(temp,Ye_i,rho,r) - YeSinkHEOS(temp,Ye_i,rho,r);   // s^-1
//        Real vdYedr = YeSource(temp,Ye_i,x,rho) - YeSink(temp,Ye_i,x,rho); // s^-1
//        Real vdYedr = YeSourceQW(temp,Ye_i,x,rho) - YeSinkQW(temp,Ye_i,x,rho); // s^-1
...
//          Real testSource = YeSourceHEOS_nan(temp,Ye_i,rho,r);
//          Real testSink   = YeSinkHEOS_nan(temp,Ye_i,rho,r);
          std::cout << " "  << std::endl;
        }

//        if(time > 0.22360646398049361 && time < 0.23 ) {
//          std::cout << "temp = " << temp << " MeV" << std::endl;
//          std::cout << "Ye_i = " << Ye_i << std::endl;
//          std::cout << "rho  = " << rho << " g/cm^3" << std::endl;
//          std::cout << "P    = " << p << " erg/cm^3" << std::endl;
...
//    std::cout << "INFLOWINNERX1 (half-step): "  << std::endl;
//    std::cout << "rho(is+1) = " << prim(IDN,ks,js,is+1) << std::endl;
//    std::cout << "rho(is)   = " << prim(IDN,ks,js,is) << std::endl;
//    std::cout << " " << std::endl;
//  } else {
//    std::cout << "INFLOWINNERX1 (full-step): "  << std::endl;
//    std::cout << "rho(is+1) = " << prim(IDN,ks,js,is+1) << std::endl;
//    std::cout << "rho(is)   = " << prim(IDN,ks,js,is) << std::endl;
//    std::cout << " " << std::endl;
//  }
...
//  if (tau > tau_v * (1.0 + tau_epsilon)) {
//    std::cout << "-------------------" << std::endl;
//    std::cout << "tau is too big:" << std::endl;
...
//  std::cout << "tau         = " << tau << std::endl;
//  std::cout << "ncycle      = " << ncycle << std::endl;
//  std::cout << "Roundncycle = " << Roundncycle << std::endl;
//  std::cout << "time        = " << time << " s" << std::endl;
...
// Performing Double NR for Yeq and Teq
  ...
  Real r_a   = pco->x1v(is);
  Real rho_a = prim(IDN,ks,js,is);
  Real Arr[2];
  Double_NR(Arr,r_a,rho_0);
...
//          std::cout << "p(is-i=" << is-i << ") (GHOST) = " << prim(IPR,k,j,is-1) << " erg/cm^3" <<std::endl;
...
void MeshBlock::UserWorkInLoop() {
  ...
  if (lid == 0) {
    local_sum = 0.0;
//    std::cout << "testrho (WorkInLoop) = " << testrho << " g/cm^3" << std::endl;
  }
...
//        std::cout << "rho_ip1 = " << rho_ip1 << std::endl;
//        std::cout << "rho_i   = " << rho_i   << std::endl;
//        std::cout << "r_ip1   = " << r_ip1   << std::endl;
//        std::cout << "r_i     = " << r_i     << std::endl;
//        std::cout << "ye_ip1  = " << ye_ip1  << std::endl;
//        std::cout << "ye_i    = " << ye_i    << std::endl;
//        std::cout << "chi_ip1 = " << Chi_ip1 << std::endl;
//        std::cout << "chi_i   = " << Chi_i   << std::endl;
//	std::cout << " " << std::endl;
      }
    }
  }
...
//    std::cout << "global_integral = " << global_integral << std::endl;
//    std::cout << "sigma_nue_n     = " << sigma_nue_n << " cm^2" << std::endl;
//    std::cout << "----------------------------------------" << std::endl;
  }

// ------------- MY STUFF --------------------
//  std::cout << "lid = " << lid << std::endl;
//  std::cout << "pmy_mesh->nblocal = " << pmy_mesh->nblocal << std::endl;
//  std::cout << "rho_0 = " << phydro->w(IDN,0,0,0) << " g/cm^3" << std::endl;
//  std::cout << "my_data(0) = " << my_data(0) << std::endl;
//  std::cout << "rho(is)    = " << phydro->w(IDN,0,0,is) << " g cm/^3, is = " << is << std::endl;
//  std::cout << "rho(ie)    = " << phydro->w(IDN,0,0,ie) << " g cm/^3, ie = " << ie << std::endl;
//  if (lid == 0 ) {
//    local_sum = 0.0;
//  } else if (lid == pmy_mesh->nblocal-1) {
//    Real global_integral;
//    MPI_Allreduce(&local_sum, &global_integral, 1,
//                  MPI_ATHENA_REAL, MPI_SUM, MPI_COMM_WORLD);
//  } else {
...
//        if(i==is) {
//          // Set first element to tau
//          user_out_var(2,k,j,i) = global_tau;
//        } else if (i==is+1) {
//          // Set second element to ghost zone density, i.e., rho_new
//          user_out_var(2,k,j,i) = phydro->w(IDN,k,j,0);
//        } else {
